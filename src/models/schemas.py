...
class AgentThoughtAndAction(BaseModel):
    """Structured output for agent reasoning and action selection."""

    thought: str = Field(..., description="The agent's reasoning for its next step")
    action: str | None = Field(None, description="The name of the tool to use, or null if finished")
    action_input: dict[str, Any] = Field(
        default_factory=dict, description="The parameters for the tool"
    )
    is_final_answer: bool = Field(
        default=False, description="True if this is the final answer, not an action"
    )
    final_answer: str | None = Field(None, description="The final answer if is_final_answer=True")


class AgentPlan(BaseModel):
    """Multi-step plan generated by reasoning engine."""

    goal: str = Field(..., description="The overall goal of the plan")
    steps: list[dict[str, Any]] = Field(..., description="Ordered list of steps to execute")
    estimated_complexity: float = Field(..., description="Complexity score 0.0-1.0")
...
class QueryClassification(BaseModel):
    ...
    complexity: str = Field(
        ..., description="SIMPLE, MODERATE, or COMPLEX", pattern="^(SIMPLE|MODERATE|COMPLEX)$"
    )
    reasoning: str = Field(..., description="Explanation for the classification")
    recommended_strategy: str = Field(..., description="Recommended reasoning strategy")
