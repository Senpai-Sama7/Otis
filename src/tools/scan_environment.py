...
class ScanEnvironmentTool(BaseTool):
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Scan completed", scan_type=scan_type, findings_count=len(result.get("findings", [])))
    ...
    async def _scan_ports(self, target: str, port_range: str) -> Dict[str, Any]:
        ...
        tasks = [check_port(port) for port in range(start_port, min(end_port + 1, start_port + 100))]
        results = await asyncio.gather(*tasks)
        
        open_ports = [
            {"port": port, "status": "open"}
            for port, is_open in zip(range(start_port, min(end_port + 1, start_port + 100)), results)
            if is_open
        ]
