...
def test_homograph_detector():
    ...
    assert details["threat_detected"] == True
...
def test_confidence_anomaly_detector():
    ...
    assert detected == False or (details and details.get("anomaly_type") is None)
...
def test_detector_detection_logic():
    ...
    for text, should_detect in test_cases:
        ...
        if should_detect:
            assert detected == True, f"Should detect homograph in: {text}"
        else:
            assert detected == False, f"Should not detect homograph in: {text}"
...
def test_encoding_detector_patterns():
    ...
    for encoded_text in [url_encoded, html_encoded, mixed_encoded]:
        detected, details = detector.detect(encoded_text)
        assert detected == True, f"Should detect encoding in: {encoded_text}"
        if details:
            assert details["threat_detected"] == True
...
def test_injection_keywords():
    ...
    for text in injection_texts:
        ...
        assert detected == True, f"Should detect injection in: {text}"
        if details:
            assert details["keyword_count"] > 0
